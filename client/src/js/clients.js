...
class Clients {
    ...
    @action populateClients = async (min, max, isClientsComponent, isActionComponent, isAnalyticsComponent) => {
        this.clients = await axios.get('http://localhost:8090/clients')
    ...
    @action transfer = async (newOwner) => {
        ...
        this.clients[index] = await axios.post('http://localhost:8090/newOwner', { newOwner: newOwner, id: this.client.id });
    ...
    @action send = async (newEmailType) => {
        let index = this.clients.indexOf(this.client)
        this.clients[index] = await axios.post('http://localhost:8090/newEmailType', { newEmailType: newEmailType, id: this.client.id });
        this.clients[index] = this.clients[index].data
        this.client = this.clients[index]
    }
    ...
    @action switchSale = async () => {
        ...
        let index = this.clients.indexOf(this.client)
        this.clients[index] = await axios.post('http://localhost:8090/sale', { newBool: newBool, id: this.client.id });
        this.clients[index] = this.clients[index].data
        this.client = this.clients[index]
    }
    ...
    @action editClient = async (index, changes, min, max) => {
        ...
        this.clients[index] = await axios.post('http://localhost:8090/qedit', changes);
        this.clients[index] = this.clients[index].data
        this.populateDisplayed(min, max)
    }
    ...
    @action addNewClient = async (name, surname, country, owner) => {
        ...
        this.clients = await axios.post('http://localhost:8090/new', newClient);
        this.clients = this.clients.data;
    }
}
